...
object Google : MailProvider {
    ...
    override suspend fun trashMail(req: MessageDeleteRequest): UnifiedEmail {
        val gMail: GMail =
            client.post("https://gmail.googleapis.com/gmail/v1/users/me/messages/${req.mailId}/trash") {
                headers { append(HttpHeaders.Authorization, "Bearer ${req.token}") }
            }.body()
        return gMail.toUnifiedMail()
    }

    @OptIn(ExperimentalCoroutinesApi::class)
    override suspend fun fetchMails(req: MessageSearchRequest) = flow<UnifiedEmail> {
        ...
        emitAll(
            messageIdList.messages.asFlow() // Convert the list of message IDs into a flow
                .flatMapMerge(concurrency = 5) { messageId -> // Fetch emails concurrently (adjust concurrency level)
                    flow {
                        val email = fetchEmailDetails(req.token, messageId.id)
